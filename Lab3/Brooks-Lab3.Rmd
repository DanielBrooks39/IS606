...
load("~/R/win-library/3.2/IS606/labs/Lab3/more/bdims.RData")
```
...
mdimsHeight <- mdims$hgt
fdimsHeight <- fdims$hgt

hist(mdimsHeight, main = "Histogram of Men's Height", xlab = "Men's Height")
hist(fdimsHeight, main = "Histogram of Women's Height", xlab = "Women's Height")
```

The mens height histogram appears to be normal, or clse to normal. Most of 
the data is contained around the mean and slowly falls off on each side
from there. The womens data on the other hand is a little bit different.
It still appears to be normal, but it is there is a lot more data contained
it the middle. So the graph will be more spread out the the mens graph.
The mens graph will be more pointy.
...
It does appear that the graph is normal. It fits the curve pretty well. The data 
is clustered in the middle of the graph but the data still falls off pretty evenly
...
qqnorm(sim_norm, main = "Simulated Plot")
qqline(sim_norm)

qqnorm(fdims$hgt, main = "Actual (Real Data) Plot")
qqline(fdims$hgt)
```
All of the points still do not fit of the line. There is still a few points that
that are on the tails of the graph that are off of the line. It appears
that there are more points on the tails that are on the line the in the 
real data graph.
...
Yes it does appear that the female height data is nearly normal. All of the plots
are very close to the niormal line.
...
```{r female_weight, eval = TRUE}
fdimsWeight <- fdims$wgt
WeightMean <- mean(fdimsWeight)
WeightSD <- sd(fdimsWeight)

hist(fdims$wgt, probability = TRUE)
x <- 40:116
y <- dnorm(x = x, mean = WeightMean, sd = WeightSD)
...
Looking at the histogram and the line of the scatter plot, I would say that this
is not normal. The graph appears to be skewed to the right. A lot more of the graph
is packed toward the left side of the graph and not evenly distributed.
...
How many wome are below 5 feet tall?
```{r below_five_feet}
ProbHgt <- pnorm(q = 152.4, mean = fhgtmean, sd = fhgtsd)
ProbHgt

ProbHgt <- sum(fdims$hgt < 152.4)/length(fdims$hgt)
ProbHgt
```

How many girls weigh more than 70 kgs?
```{r weigh_more_than_70}
ProbWgt <- 1 - pnorm(q = 70, mean = WeightMean, sd = WeightSD)
ProbWgt

ProbWgt <- sum(fdims$wgt > 70)/length(fdims$wgt)
ProbWgt
```
* * *
...
ANSWER:
  ...
  The have a step wise pattern because they are not continuous distributions.
They are discrete. That is the reason why there are steps, because each step
is a different variable in the discrete distribution. Continuous distributions
...
```{r knee_diameter, eval = TRUE}
knee <- fdims$kne.di
kneeMean <- mean(knee)
kneeSD <- sd(knee)

hist(knee, probability = TRUE)
x <- 15:24
y <- dnorm(x = x, mean = kneeMean, sd = kneeSD)
lines(x = x, y = y, col = "blue")
```

It is skweded to the right.