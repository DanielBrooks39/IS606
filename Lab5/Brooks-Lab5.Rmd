...
load("~/r/win-library/3.2/IS606/labs/lab5/more/nc.RData")
```
...
* The cases in the data set are, the fathers age, the mothers age, the maturity status of the mother, the length of the pregnancy, the classification of the birth, number of hpsital vists, marital status  of the mother, amount of wieght gained, babies weight, classification of babies wieght, gender of the baby, whether the mother is a smoker or not, whether the mom is white or not. There are 13 cases in our sample.
...
``` {r box_plot, eval=TRUE}
library(useful)
ggplot(nc, aes(x=habit, y= weight)) + geom_boxplot() + ggtitle("Habit vs Weight")
```
* I can see that the average weight for the non smokers are a lot higher than that of smokers. The data for the non-smokers is also mpre spread out than the sokers data. That was interesting to see because I would thinking would cause a greater variety in weight but that seems to be a misconseption.
...
```
* There is 873 nom-smokers and 126 smokers. That is enough for an hypothesis test.

4.  Write the hypotheses for testing if the average weights of babies born to 
smoking and non-smoking mothers are different.

* I hypothesize that the average wieght of a baby born by a non-smoker will be greater that the average wieght of a baby born by a smoker.
...
```{r inf-weight-habit-ci-mine, eval=TRUE, tidy=TRUE}
...
```{r inf-weight-habit-ci-95, eval=TRUE, tidy=TRUE}
...
```{r inf-weight-habit-ci-90, eval=TRUE, tidy=TRUE}
inference(y = nc$weeks, est = "mean", type = "ci", null = 0, 
...
```{r inf-weight-habit-ht-95, eval=TRUE, tidy=TRUE}
...
mature <- subset(nc, mature != "younger mom")

max(young$mage)
min(mature$mage)
```
* The age cutoff for a younger mom is 34 and mature women are 35 and above.
...
* I am hypothesizing that the mature moms vist the doctor less than the younger moms
```{r ht_status_vs_doctor_visits}
inference(y = nc$visits, x = nc$mature, est = "mean", type = "ht", null = 0, 
          alternative = "less", method = "theoretical")
```

* The p-value of the hypothesis test was .925. That means that we cannot accept the null hypothesis and we have to accept the alternative hyopthesis. In non-math speak, women labled as mature women do not go to the doctors less than those labled younger women.