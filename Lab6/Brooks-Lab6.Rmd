...
* It appears that this data is derived from population data. Thy sampled people from all over the world. The people polled also appear to be reandomly picked. I do not see any sort of startafication or subsetting at all that woul lend this to sample data.
...
* We would have to assume that there was no selection biasis at all and no grouping of the people at all. It would have to be a random sample.    
...
load("~/R/win-library/3.2/IS606/labs/Lab6/more/atheism.RData")
```

3.  What does each row of Table 6 correspond to? What does each row of 
    `atheism` correspond to?
    
* Each row of table six represents a country that was sampled in the survey. It also represents the break down by % of the number of people that are either religious, atheist, or not religious. The atheisim table represents the individual responses for the survey. one row represents one person from one country. 
...
atheist <- subset(us12, response == "atheist")
nonatheists <- subset(us12, response != "atheist")

Natheist <- nrow(atheist)
Nnonatheist <- nrow(nonatheists)
total <- nrow(us12)

Peratheist <- (Natheist/total)
Peratheist
Pernonatheist <- (Nnonatheist/total)
Pernonatheist
```
* No the numbers do not add up. The reason why they do not add up is because there are more than just two responses. The non-ahteist people are broken up into two groups. Also there is a did not respond column as well for the survey. The numbers are indeed close, just not exactly right.
...
* The conditions are, it needs to be a simple random sample, and the sample size needs to be sufficiently large(enough to get at least 10 success and ten failures).

* I think that the conditions are met to make a statistical inference. We have over 1000 observations with 50 sucesses. That is more than enough for the inference.
...
```{r Pakistan}
pakistan <- subset(atheism, nationality == "Pakistan" & year == "2012")
atheist <- subset(pakistan, response == "atheist")
nonatheists <- subset(pakistan, response != "atheist")
...
inference(pakistan$response, est = "proportion", type = "ci", method = "theoretical", 
          success = "atheist")
phat <- .02
n <- 2704
ME <- sqrt((phat*(1-phat))/n)
...
atheist <- subset(brazil, response == "atheist")
nonatheists <- subset(brazil, response != "atheist")
...
* There is enough for both countries to continue with the inferences.
...
* It appears that the relationship between p and me is a half circle.  The max of the circle is around p = .5. It makes sense because the probability of a sucess is just 1-failure so that is whay it makes a cirlce type pattern.
...
* The distribution is approximately normal. The center of the data at .10 which is the value of p hat.
...
hist(p_hats, main = "p = 0.1, n = 400", xlim = c(0, 0.20))
```

```{r sim-1040, eval=TRUE}
p <- 0.2
n <- 1040
p_hats <- rep(0, 5000)
...
hist(p_hats, main = "p = 0.2, n = 1040", xlim = c(.10, 0.30))
```

```{r sim-4002, eval=TRUE}
p <- 0.2
n <- 400
p_hats <- rep(0, 5000)
...
hist(p_hats, main = "p = 0.2, n = 400", xlim = c(.10, 0.30))
```
...
* Yes I do feel that it would be OK to continue on with the inference. The graphs do appear to be normal ad there is more than enough data to satisfy the equations.
...
spain05 <- inference(y = spain05$response, est = "proportion", type = "ci", method = "theoretical", 
          success = "atheist")
spain12 <- inference(y = spain12$response, est = "proportion", type = "ci", method = "theoretical", 
          success = "atheist")
```
* It appears that the entire confidence interval for 2012 is contained within the interval for 2005. That shows that there has been no change. There about the same amount of atheists in 2005 and 2012 in spain  
...
us12 <- inference(y = us12$response, est = "proportion", type = "ci", method = "theoretical", 
          success = "atheist")
usa05 <- inference(y = usa05$response, est = "proportion", type = "ci", method = "theoretical", 
          success = "atheist")
```
* I would say that there is not significant evidence to say that the amount of atheism in the United States has changed. The intervals are pretty close together.
...
* I would expect to see a change in about 5 % of them. That is because the is a 5 % error that comes along with the confidence interval. We are saying that there is a 5 % chance of making a type 1 error  with thos inference.
...
* Looking at the graph of the standard error to the p hat value. it appears that the p hat should be at about .05. that means that there needs to be at least 500 participants in the sample to fall within the guidelines.