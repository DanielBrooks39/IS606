...
load("~/R/win-library/3.2/IS606/labs/Lab4a/more/ames.RData")
```
...
* The data appears to be a little skewed to the right. It might be able to be called nearly normal just because mostly all of the data is cetered around 1000, but I would say it is a little bit skewed to the right.
...
* This data appears to be skewed right. I would still say that this data is nearly normal, just like the population data was. It has a little bit of extra data on the far right side but it looks nearly normal.
...
* The two sample means are relatively close together. Usually within 100 of each other

***

``` {r more_sample, eval = TRUE}
...
* The two means are even closer together. I would say samp2 (sample size of 1000) would be the closest to the population mean because the sample size is large. The more things items in your sample size the closer you will get to your acutal population mean
...
* There are 5000 elements in sample_mean50. This data is normal. It is a nice bell curve. The center of the curve is at 1500. I believe that was the population mean for the data that was mentioned earlier.I would expect the distrinution to become even more normal with the more data that we get. The mean will ge closer and closer toeh 1500 (The population mean).
...
* There are a total of 100 elements in sample_means_small. Each element contains a mean of a sample of 50 elements from the population dataset.
...
```{r plot-samps, eval=TRUE, tidy = TRUE}
par(mfrow = c(3, 1))
...
* As the sample size gets bigger, the mean becomes closer to 1500 (Population mean). Also, the data becomes less spread out. This is probably due to the fact that the disturibution is becoming closer to the population distribution.
...
* I would say the mean would be between 175000 and 185000 based off of the variety of samples I have seen.
...
* I would guess that the mean of the distribution be around 180000. The center of the graph is around the 180000 mark.
...
* The distribution of 150s is more normal that the sample of 50s. Also, the 50s is more spread out than the 150s. I can see that the mean is a little bit more than 180000 because there is a little more data on the right side of 180000.
...
* The 150 samples has a smaller spread. It looks to go from about 160000 to about 200000. The 50s ranges from about 150000 to about 210000. I would want the data to be less spread out when I am looking for the true value of the population. There is less of a chance of error.
