...
load("~/R/win-library/3.2/IS606/labs/lab7/more/mlb11.RData")
```
...
* I would use scatter plot to show the relationship between the two variables.
```{r}
plot(x=mlb11$at_bats, y=mlb11$runs, xlab="At_Bats", ylab="Runs")
```

* I would say that the relationship does look linear. There is a increase of
the number of runs as the at bats increase. I would be comfortable using a 
linear relationship to predict the number of runs based off of the number
of at bats.
...
* The graph does appear to have a linear trend. As the number of at bats increase,
the number of runs increase as well. It is leaning towards the right so it is a
positive relationship between the two of them. The data seems a little spread out
but, it seems to have a decent correlation.
...
* The lowest sum of sqaures value that I got was 132227.
...
```{r}
plot(x=mlb11$homeruns, mlb11$runs, xlab="Homeruns", ylab="Runs")
...
* The slope tells us that every one more homerun that is hit, counts for
1.8345 more runs
...
```{r}
runs <- -2789.2429 + .6305 * 5578
runs
```

* This is an underestimation because it is above the regression line.
It appears to be underextimated by about 27 runs.
...
* The residuals apear to be relatively evenly spread across the horizontal line.
It shows that the points are all about the same distance away from the regression
line.
...
* I would say yes. Th ehistogram showed it looked relatively normal and the 
QQ plot shows a relatively linear residual distribution as well.
...
```{r}
plot(x=mlb11$bat_avg, y=mlb11$runs, xlab="Bating Average", ylab="Runs")
```

* It does appear to have a linear relationship.
...
summary(m3)
```
* The R$^2$ for the runs vs at_bats was .3729
* The R$^2$ for the runs vs batting avg was .6561
* It is a better predictor because the residuals are less
...
* The R$^2$ for the runs vs hits was .6419
* The R$^2$ for the runs vs homeruns was .6266
* The R$^2$ for the runs vs strikeouts was .1694
* The R$^2$ for the runs vs stolen bases was .002914
* The R$^2$ for the runs vs wins was .361

* The variable the predicts the runs the rest is the batting average.
...
* The R$^2$ for the runs vs On base % was .8491
* The R$^2$ for the runs vs slugging % was .8969
* The R$^2$ for the runs vs OBS was .9349

* The oBS is the best predictor
...
* Everything appears to pass all of the tests for linearlity.