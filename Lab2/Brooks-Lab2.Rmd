...
```{r load_data_Get_Information}
load("~/R/win-library/3.2/IS606/labs/Lab2/more/kobe.RData")
head(kobe)
```
...
ANSWER: It means that he has a streak of one. That means he made his first shot 
and then missed the next shot he took. A streak of two means, he made two shots 
in a row and missed his third one.
...
ANSWER: His streak would be skewed right. It has most of the data on the left side 
of the graph. His typical streak is between 0 and 1. His longest streak was 4 in
the NBA finals.
...
sim_fair_coin <- sample(outcomes, size = 1, replace = TRUE)
```
...
sim_unfair_coin <- sample(outcomes, size = 1000, replace = TRUE, prob = c(0.4, 0.6))
```
...
3.  In your simulation of flipping the unfair coin 100 times, how many flips 
    came up heads?
```{r sim_table_unfair_coin, eval = TRUE}
...
4.  What change needs to be made to the `sample` function so that it reflects a 
    ...
    `sim_basket`.
```{r sim_basket, eval=TRUE}
FG_percent <- sample(outcomes, size = 133, replace = TRUE, prob = c(0.45, 0.55))
table(FG_percent)
```
...
```{r calc_streak, eval=TRUE}
streak_basket <- calc_streak(sim_basket)
barplot(table(streak_basket))
```
The Distribution is just like Kobe's. The data is very much skewed to the right.
Most of the data is around the 0-1 range.The longest streak that I have seen 
during the simulations, is 8.
...
When I run the simulation the second or third or nth time, I would expect
the general shape of the graph to remain the same. Most of the data will
remian within 0-1. The thing that will change, is the largest streak in the
distribution. Due to the fact that a make is just a little bit higher than
a miss, it is rather difficult to get to the larger streak numbers everytime
you run the simulation. That means that the largest value will change a lot
but the data towards the left side of the graph will remain close to the same.
...
barplot(table(kobe_streak))
```
It would appear that the two streak graphs are pretty similar. Both plots
are skewed to the right. KObe appears to have a lot more data towards
the left side of the graph (between 0-1), while the simulated shooter
has more longer streaks, but overall, the two graphs do look pretty
similar.
